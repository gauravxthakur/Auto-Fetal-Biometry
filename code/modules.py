...
def ellip_fill(in_folder, out_folder):
    """
    Fill the original label to create segmentation mask.
    """
...
def hausdorff_d(img1, img2):
    ...
    # Safety check: If either image is empty, return a high distance or 0
    if not contours1 or not contours2:
        return 0 

    # Calculate distance transform of the first image
    # This creates a map where each pixel value is its distance to the nearest edge
    dist_map = cv2.distanceTransform(cv2.bitwise_not(img1), cv2.DIST_L2, 5)
    
    # The Hausdorff distance is the maximum distance from any point on contour2 to contour1
    # We sample the points from contour2 and check their values on the distance map
    dists = [dist_map[p[0][1], p[0][0]] for p in contours2[0]]
